...
static int viewRequests(const char* s);
...
static int loginUser(const char* s, const string& username, const string& password) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        unsigned char* name1 = const_cast<unsigned char*>(name);
        unsigned char* pass1 = const_cast<unsigned char*>(pass);
        char* name2 = reinterpret_cast<char*>(name1);
        char* pass2 = reinterpret_cast<char*>(pass1);
        const char* name3 = username.c_str();
        const char* pass3 = password.c_str();
        int result1, result2;
        result1 = strcmp(name2, name3);
        result2 = strcmp(pass2, pass3);

        if (result1 == 0 && result2==0) {
            cout << "It exists in database! Login succesful!" << endl;
